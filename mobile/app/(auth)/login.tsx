...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setErrors({
        form: err instanceof Error ? err.message : "Connexion impossible",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    >
      ...
      >
        <View className="items-center mb-10">
          ...
          <Text className="text-dark-400 mt-2">Connectez-vous</Text>
        </View>
        ...
        <View className="flex-row justify-center mt-6">
          <Text className="text-dark-400">Pas encore de compte ? </Text>
          <Link href="/(auth)/register" asChild>
            <TouchableOpacity>
              <Text className="text-indigo-400 font-semibold">S'inscrire</Text>
            </TouchableOpacity>
          </Link>
        </View>
      </ScrollView>
